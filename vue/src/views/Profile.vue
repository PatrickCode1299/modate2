...
axiosClient.post("/profile",{email:user_mail}).then((response=>{
    if(response.data.info==="false"){
       ...
       incomplete=sessionStorage.getItem('INCOMPLETE');
       personal_info.u_incomplete=sessionStorage.getItem('INCOMPLETE');
    }else{
       info.info_value="false";
        ...
        sessionStorage.setItem('COVER_TEXT',response.data.coverText);
        if(response.data.coverPhoto===null){
            ...
            cover_bg=sessionStorage.getItem('COVER_PHOTO');
            ...
            if(response.data.coverText==null){
               ...
               info.cover_text=sessionStorage.getItem('COVER_TEXT');
            }
            
            
           
        }
        personal_info.u_first_name=sessionStorage.getItem('FIRSTNAME');
        personal_info.u_last_name=sessionStorage.getItem('LASTNAME');
        personal_info.u_location=sessionStorage.getItem('LOCATION');
        personal_info.u_phone_number=sessionStorage.getItem('PHONE');
        personal_info.u_incomplete=sessionStorage.getItem('INCOMPLETE');
        user_profile_pic=sessionStorage.getItem('PICTURE');
   ...
   <div class="container user-profile p-2">
    <div :style="{backgroundImage:info.bgUrl}" class="images p-4">
        ...
        <div class="edit-profile d-flex justify-content-flex-start">
            <button @click="gotoProfileHeader" class="btn edit-btn btn-sm btn-success">Edit Profile</button>
        </div>
        <div class="user-pic-name">

            <img v-if="personal_info.u_incomplete === 'true'" src="../landing/blondie.jpg" class="user-profile-img" />
            <img v-else :src="`http://localhost:8000/storage/${user_profile_pic}`" class="user-profile-img" />
    ...
    <div class="user-info-card p-2">
        <div class="bg-white shadow-sm user-info-card p-2">
            <div class="heading-edit-btn">
                ...
                <div class="edit-btn p-2">
                    <button v-if="personal_info.u_incomplete === 'true'" class="btn btn-default edit btn-sm" @click="gotoEdit">Edit</button>
                    <button v-else class="btn btn-default edit btn-sm" @click="gotoSupport">Modify</button>
                </div>
            </div>
            <div class="all-user-info p-4">
                <div v-if="personal_info.u_incomplete === 'true'" class="incomplete-profile">
                    <h2 class="fs-2 text-bold text-center">Your profile is not visible to other users.</h2>
                    <p class="text-center">Click on the Edit button above to complete your profile</p>
                </div>
