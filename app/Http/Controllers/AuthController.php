...
public function sendUpdateProfileLink(Request $request){
    ...
    $reset_link=[
      'welcome_text'=>"You can update your profile via this mail, $email",
       ...
       'call_to_action'=>'This link expires today, do not share this mail with anyone..',
       'url' => url("https://hexarex.com/update/".base64_encode($current_date)."/".base64_encode($email).""),
       'conclusion' => "From hexarex.com",
       'id'  => 1
    ];
    Notification::send($recipient, new ResetPassword($reset_link));
...
public function updateDetails(Request $request){
  ...
  $ad_owner=$request->input('email');
  $target_location=$request->input('target_location');
  $minimum_target_age=$request->input('minimum_target_age');
  $max_target_age=$request->input('max_target_age');
  $target_post=$request->input('postid');
  $ad_duration=$request->input('ad_duration');
  $ad_amount=$request->input('amount_paid');
 AdvertDetail::create([
"ad_owner" => $ad_owner,
