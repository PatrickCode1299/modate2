...
$find_all_channels_of_who_user_follows_post = DB::table('channel_posts')
    ...
    ->where(function ($query) use ($find_user_choice, $current_user_location) {
        // Check if user follows the channel OR if the post is from users within the same location
        $query->whereIn('channel_posts.email', $find_user_choice)
              ->orWhere('users.location', $current_user_location);
    })
    ->select(
        'channel_posts.name',
