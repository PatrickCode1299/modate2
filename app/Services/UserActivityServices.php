...
class UserActivityServices{
    ...
    foreach($db_post as $get_post){
     $post_id=$get_post->postid;
      ...
      array_push($store_all_post,["name"=>$get_post->name, "caption"=>$get_post->caption, "post_img1"=>$get_post->post_img1, "post_img2"=>$get_post->post_img2, "post_img3"=>$get_post->post_img3,"post_img4"=>$get_post->post_img4,"video"=>$get_post->video, "date"=>$get_post->created_at,"avatar"=>$get_post->profile_picture,"email"=>$get_post->email,"id"=>$get_post->id,"isReply"=> $get_post->isReply, "likes"=>count($post_like_collection),"comments"=>count($post_comments_collection), "shares"=>count($post_shares_collection)]);
    ...
    public function findChannelVideos($current_user){
     ...
     ->where('channel_posts.video','!=','')->whereIn('channel_posts.email',$keep_all_persons_who_user_follow)->select('channel_posts.name','channel_posts.caption','channel_posts.created_at','channel_posts.post_img1','channel_posts.post_img2','channel_posts.post_img3','channel_posts.post_img4','channel_posts.video','channel_posts.postid', 'channel_posts.isReply', 'users.profile_picture','users.first_name','users.last_name','channels.channel_bio',   'channel_posts.email','channel_posts.id')->latest()->get();
     $store_all_post=array();
